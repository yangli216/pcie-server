...
export default {
  name: 'XiaoshanFunctionUsageView',
  components: {
    AdminFilterBar,
    ChartPanel,
    MetricCard,
  ...
  methods: {
    ...
    async loadRefs() {
      try {
        if (this.organizationLocked) {
          this.moduleOptions = await http.get('/admin/api/xiaoshan-analytics/function-modules') || []
          return
        }
        const [regions, hisOrgs, mods] = await Promise.all([
          fetchRegions({ sdStatus: '1' }),
          fetchHisOrgOptions(),
          http.get('/admin/api/xiaoshan-analytics/function-modules')
        ])
    ...
    async search() {
      ...
      try {
        ...
        const data = await http.get('/admin/api/xiaoshan-analytics/function-usage', { params })
        this.response = data || {}
    ...
    async exportData() {
      ...
      try {
        ...
        if (!params.hisOrgId) delete params.hisOrgId
        if (!params.functionModules || params.functionModules.length === 0) delete params.functionModules
        const blob = await http.get('/admin/api/xiaoshan-analytics/function-usage/export', { params, responseType: 'blob' })
        this.downloadBlob(blob, '萧山辅诊功能统计_' + new Date().toISOString().slice(0, 10) + '.xlsx')
      } catch (error) {
