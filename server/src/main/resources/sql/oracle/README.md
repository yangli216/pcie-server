...
3. 新建库仍采用“目标 schema 初始化/重建 + 重跑 `init.sql`”；当前只定向保留 `update_his_org_statistics.sql` 用于补齐 HIS 统计契约，不保留慢病表升级脚本
...
两慢病正式数据已改由 HIS Adapter 进入真实业务系统，当前初始化基线不再创建 `c_ai_chronic_followup` 和 `c_ai_chronic_artifact`。已部署库中的历史冗余表由 DBA 在备份和变更审批后一次性删除。
